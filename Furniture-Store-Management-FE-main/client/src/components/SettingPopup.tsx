...
export default function SettingPopup(
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black bg-opacity-50 flex items-center justify-center">
      <div className="relative popup bg-[#fff] max-w-[400px] w-full p-4 overflow-hidden rounded-lg ">
        <CloseIcon className="absolute top-2 right-2 cursor-pointer hover:bg-slate-300 rounded-full" onClick={onClose} />
        <div className="flex flex-row gap-2 items-center">
          <FormGroup>
            <FormControlLabel
              control={<MaterialUISwitch sx={{ m: 1 }} onClick={handleSetDarkMode} checked={darkmode === 'dark' ? true : false} />}
              label="Dark Mode"
            />
          </FormGroup>
        </div>
